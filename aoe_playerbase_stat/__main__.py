...
def main():
    ...
    # We need a function here that checks for the presence of a file in a file path
    # for the different stages we can have a namedtuple that give the settings for
    # different stages
    # `Analyse stage` with existing parquet file should be fine
    # `Plot stage` with existing parquet file should be fine
    # `Process stage` without files in GLOBAL_SETTINGS['FILESYSTEM']['TEMPORARY_CACHE_FOLDER']
    #  should not work and give a warning to run `collect stage``
    # `Collect stage` should throw warning early if filesystem is not setup for saving the files
    # so we should just check if we can create the files or we should actually try to create
    # tempfiles for this stage
    if (

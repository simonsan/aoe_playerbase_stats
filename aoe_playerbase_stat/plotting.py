...
from aoe_playerbase_stat.utils.dataframes import prepare_dataframes
...
def plotting():
    ...
    range_slider.js_link(
        "value", activity_plot.y_range, "start", attr_selector=0
    )
    range_slider.js_link(
        "value", activity_plot.y_range, "end", attr_selector=1
    )
    ...
    callback = CustomJS(
        args=dict(
            name_dict=activity_group["activity"], multi_choice=multi_choice
        ),
        code="""

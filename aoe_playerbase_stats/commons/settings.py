...
GLOBAL_SETTINGS = {
    ...
    "FILESYSTEM": {
        "AOC_REF_DATA_FILE_PATH": os.path.join(
            __TEMPORARY_DATA_FOLDER, "aoc_ref_data.json"
        ),
        "ARCHIVED_CACHE_FOLDER": os.path.join(
            __TEMPORARY_DATA_FOLDER, "cache/archive/"
        ),
        "DATA_FOLDER": __DATA_FOLDER,
        "DATASET_FILE_PATH": os.path.join(__DATA_FOLDER, "dataset.json"),
        "TEMPORARY_CACHE_FOLDER": os.path.join(
            __TEMPORARY_DATA_FOLDER, "cache/"
        ),  # previously: CACHE_PATH, CACHE_FOLDER
        "TEMPORARY_DATA_FOLDER": __TEMPORARY_DATA_FOLDER,
        "PARQUET_FILE_PATH": os.path.join(__DATA_FOLDER, "dataframe.br.parq"),
        "PLOT_OUTPUT_FILE_PATH": os.path.join(
            __WEB_FOLDER, "index.html"
        ),  # previously: PLOT_OUTPUT
